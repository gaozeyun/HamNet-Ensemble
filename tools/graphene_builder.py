...
def build_bilayer_graphene(stacking='AA', d_cc=1.42, d_vdw=3.35, crystal='graphene'):
    ...
    if stacking.upper() == 'AA':
        ...
        raise ValueError("Only supports 'AA' or 'AB' stacking modes.")
...
def apply_wrinkles_auto_supercell(atoms, target_wavelength=50.0, amplitude=1.0, n_waves=(1, 0), wave_type='1D', phase=0.0):
    ...
    if rep_x > 1 or rep_y > 1:
            print(f"[*] Triggering adaptive supercell expansion: current a={len_a:.2f} Å, b={len_b:.2f} Å.")
            print(f"    Auto-expanding to {rep_x} x {rep_y} supercell to accommodate physical wavelength of {target_wavelength} Å.")
            working_atoms = atoms * (rep_x, rep_y, 1)
    else:
        print(f"[*] Unit cell dimensions sufficient: current a={len_a:.2f} Å, b={len_b:.2f} Å. No additional expansion needed.")
    ...
    if eps_xx < 0 or eps_yy < 0:
        print(f"[*] Physical equal-arc-length correction: applying pre-compression strain eps_xx={eps_xx*100:.2f}%, eps_yy={eps_yy*100:.2f}%")
    ...
    if isinstance(phase, (int, float)):
        ...
        raise ValueError("phase must be a single number, or a tuple/list of length 2 (e.g., (0.0, 1.57))")  
    if wave_type.upper() == '1D':
        delta_z = amplitude * np.sin(2 * np.pi * (n1 * u + n2 * v) + phase_u)
    elif wave_type.upper() == '2D':
        if n1 == 0 or n2 == 0:
            raise ValueError("For 2D wrinkles, both components of n_waves cannot be zero.")
        delta_z = amplitude * np.sin(2 * np.pi * n1 * u + phase_u) * np.sin(2 * np.pi * n2 * v + phase_v)
    else:
        raise ValueError("wave_type must be '1D' or '2D'")
...
def main():
    parser = argparse.ArgumentParser(
        description="Graphene Structure Builder",
        formatter_class=argparse.RawDescriptionHelpFormatter
    )

    parser.add_argument("-t", "--task", type=str, default="twisted", choices=["single", "bilayer", "twisted"], help="Task type: single (monolayer), bilayer (bilayer), twisted (twisted bilayer).")
    parser.add_argument("-o", "--outdir", type=str, default="./input", help="Output directory path.")
    parser.add_argument("-c", "--crystal", type=str, default="graphene", help="Crystal type: graphene (c-axis=30Å) or graphite (c-axis=2*d_vdw).")

    parser.add_argument("-s", "--stacking", type=str, default="AA", choices=["AA", "AB", "aa", "ab"], help="Initial stacking mode ('AA' or 'AB').")
    parser.add_argument("--d_cc", type=float, default=1.42, help="Carbon-carbon bond length (Å).")
    parser.add_argument("--d_vdw", type=float, default=3.35, help="Interlayer distance (Å).")
    parser.add_argument("-m", type=int, default=None, help="Twist parameter m (must be coprime with n).")
    parser.add_argument("-n", type=int, default=None, help="Twist parameter n (must be coprime with m).")

    parser.add_argument("--eps_xx", type=float, default=0.0, help="In-plane strain in x-direction (e.g., 0.01 for 1%% tensile strain).")
    parser.add_argument("--eps_yy", type=float, default=0.0, help="In-plane strain in y-direction.")

    parser.add_argument("--w_amp", type=float, default=0.0, help="Wrinkle amplitude (Å), set >0 to apply wrinkles.")
    parser.add_argument("--w_lambda", type=float, default=50.0, help="Wrinkle target wavelength (Å).")
    parser.add_argument("--w_type", type=str, default="1D", choices=["1D", "2D"], help="Wrinkle type: '1D' (stripes) or '2D' (egg-carton).")
    args = parser.parse_args()

    print(f"[*] Starting build task [{args.task}] ...")
    try:
        is_graphite = args.crystal.lower() == "graphite"
        crystal_name = "Graphite" if is_graphite else "Graphene"
        print(f"    Crystal type: {crystal_name}")

        if args.task == "single":
            print(f"    Parameters: bond length={args.d_cc} Å")
            atoms = build_graphene_primitive(d_cc=args.d_cc, crystal=args.crystal, d_vdw=args.d_vdw)
        elif args.task == "bilayer":
            print(f"    Parameters: stacking={args.stacking.upper()}, bond length={args.d_cc} Å, interlayer distance={args.d_vdw} Å")
            ...
            if args.m is None or args.n is None:
                parser.error("Task 'twisted' requires both -m and -n parameters.")
            if math.gcd(args.m, args.n) != 1:
                print(f"[-] Error: m={args.m} and n={args.n} are not coprime")
                exit(1)
            print(f"    Parameters: m={args.m}, n={args.n}, initial stacking={args.stacking.upper()}")
            print(f"    Bond length={args.d_cc} Å, interlayer distance={args.d_vdw} Å")
        ...
        if args.eps_xx != 0.0 or args.eps_yy != 0.0:
            print(f"[*] Applying in-plane strain: eps_xx={args.eps_xx}, eps_yy={args.eps_yy}")
            atoms = apply_inplane_strain(atoms, eps_xx=args.eps_xx, eps_yy=args.eps_yy)
        if args.w_amp > 0.0:
            print(f"[*] Applying out-of-plane wrinkles: amplitude={args.w_amp} Å, target wavelength={args.w_lambda} Å, type={args.w_type}")
        ...
        print(f"[+] Build successful! Total atoms: {len(atoms)}")
        ...
        print(f"[+] Files successfully exported to directory: {os.path.abspath(args.outdir)}")
