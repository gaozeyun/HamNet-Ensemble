...
class DeepHE3Kernel:
    ...
    def eval(self, config, debug=False):
        ...
        if use_ensemble:
            S_pred_list = []
            H_mean = [None for _ in range(len(dataset))]
            H_var = [None for _ in range(len(dataset))]
            model_count = 0
            model_weights = None
            total_sum_weights = 1e-8
            if eval_config.ensemble_method == 'weighted':
                if eval_config.ensemble_weights:
                    weights = [float(w) for w in eval_config.ensemble_weights.split(',')]
                    if len(weights) != len(model_path_list):
                        raise ValueError("The model number is not match with the weights number")
                    model_weights = weights
                else:
                    raise ValueError("Set the model_weights")
            elif eval_config.ensemble_method == 'average':
                model_weights = [1.0] * len(model_path_list)
            else:
                raise ValueError(f"Unsupported ensemble_method: {eval_config.ensemble_method}. Please use 'none', 'average' or 'weighted'.")
        ###
        ...
        for index_model, model_path in enumerate(model_path_list):
                ...
                for index_stru, data in iterable:
                    ...
                    if use_ensemble:
                        if model_count == 0:
                            ...
                            H_var[index_stru] = torch.zeros_like(H_pred.real)
                        else:
                            r = weight / (total_sum_weights + weight)
                            delta = H_pred - H_mean[index_stru]
                            H_mean[index_stru] += r * delta
                            delta_new = delta * (1.0 - r)
                            H_var[index_stru] += r * ((delta * delta_new.conj()).real - H_var[index_stru])
                            H_var[index_stru] = torch.clamp(H_var[index_stru],min=0.0)
            ...
            if use_ensemble:
                ...
                print(f'Finished evaluating model {model_count} on all structures')
                for H_dict, data in zip(H_pred_list, dataset):
                    outdir = os.path.join(eval_config.out_dir, data.stru_id)
                    if outdir not in created_dirs:
                        os.makedirs(outdir, exist_ok=True)
                        created_dirs.add(outdir)
                    print(f'Writing model {model_count} output to "{data.stru_id}/hamiltonians_pred_{index_model}.h5"')
                    with h5py.File(os.path.join(eval_config.out_dir, f'{data.stru_id}/hamiltonians_pred_{index_model}.h5'), 'w') as f:
        ...
        if use_ensemble:
            ...
            for index_stru, data in enumerate(dataset):
                batch = collate([data])
                H_var[index_stru] = torch.sqrt(H_var[index_stru])
                self.update_hopping(H_pred_list[index_stru], H_mean[index_stru], batch.x, batch.edge_index, batch.edge_key, debug=debug)
                self.update_hopping_var(S_pred_list[index_stru], H_var[index_stru], batch.x, batch.edge_index, batch.edge_key, debug=debug)
                if not eval_config.inference: 
                    if not eval_config.test_only: print(f'Saving ensembled output on structure "{data.stru_id}" to test_result.h5')
                    self.save_test_result(batch, H_mean[index_stru], h5_fp)
        ###
        ...
        if eval_config.only_ij:
            ...
            if use_ensemble:
                self.convert_ijji_hamiltonians(S_pred_list)
        ...
        if not eval_config.test_only:
            ...
            if use_ensemble:
                for S_dict, data in zip(S_pred_list, dataset):
                    outdir = os.path.join(eval_config.out_dir, data.stru_id)
                    with h5py.File(os.path.join(eval_config.out_dir, f'{data.stru_id}/hamiltonians_std.h5'), 'w') as f:
    ...
    def update_hopping_var(self, H_prev, H_pred, node_attr, edge_index, edge_key, debug=False):
        ...
        for index_edge in range(edge_index.shape[1]):
            ...
            if key_term not in H_prev.keys():
                fill = 0 if debug else np.nan
                if self.dataset_info.spinful:
                    init = module.full((atom_num_orbitals[i] * 2, atom_num_orbitals[j] * 2), fill, dtype=dtype)
                else:
                    init = module.full((atom_num_orbitals[i], atom_num_orbitals[j]), fill, dtype=dtype)
                H_prev[key_term] = init
            N_M_str_edge = f'{self.dataset_info.index_to_Z[i].item()} {self.dataset_info.index_to_Z[j].item()}'
